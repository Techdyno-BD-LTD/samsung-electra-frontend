...
export default function AuctionFilterPanel({ filteringAttributes = [] }: AuctionFilterPanelProps) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-md border border-slate-200 bg-white px-4 py-4">
        ...
        >
          <h3 className="text-[24px] font-semibold text-slate-800">Bid Status</h3>
          <HiChevronUp className={`h-4 w-4 text-slate-400 transition-transform ${statusOpen ? "" : "rotate-180"}`} />
        </div>
        <div className="mt-2 h-[2.5px] w-[50%] rounded-full bg-gradient-to-r from-[#2B7FE8] via-[#2B7FE8]/60 to-transparent" />
        
        <div className={`overflow-hidden transition-all duration-300 ${statusOpen ? "mt-4 max-h-[250px] opacity-100" : "max-h-0 opacity-0"}`}>
          <div className="flex flex-col gap-2.5">
            {[
              { value: 'all', label: 'All Statuses' },
              { value: 'live', label: 'Live Bids' },
              { value: 'upcoming', label: 'Upcoming' },
              { value: 'closed', label: 'Closed' },
            ].map((s) => {
              const checked = selectedStatus === s.value;
              return (
                <label key={s.value} className="flex cursor-pointer items-center gap-2.5 text-slate-600 hover:text-slate-900">
                  <input
                    type="radio"
                    name="status"
                    value={s.value}
                    ...
                    className="h-4 w-4 text-[#2B7FE8] border-slate-300 focus:ring-[#2B7FE8]"
                  />
