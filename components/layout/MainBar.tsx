...
export default function MainBar() {
  ...
  const compareTotalCount = useAppSelector(
    (state) => state.compare.slots.filter(Boolean).length,
  );
    ...
    const timer = setTimeout(async () => {
      try {
        const response = await fetch(
          `/api/products/search?name=${encodeURIComponent(searchQuery)}`,
        );
        if (response.ok) {
          const payload = await response.json();
      ...
      <div className="lg:w-10/12 w-full mx-auto">
        <div className="flex items-center justify-between py-4">
            ...
            <div className="relative">
              <div className="relative flex items-stretch h-12 border-2 border-[#0054A6] rounded-lg">
                ...
                <button
                  ...
                  className="flex items-center justify-center w-14 bg-[#266BF9] text-white hover:bg-blue-800 transition-colors rounded-r-md"
                >
          ...
          <div className="flex items-center gap-4">
            ...
            <Link
              href="/wishlist"
              className="relative flex items-center gap-3 tracking-tight font-medium text-[#ffffff] hover:opacity-80 transition"
            >
              <div className="relative flex items-center justify-center">
                ...
                {mounted && wishlistTotalCount > 0 && (
                  <span className="absolute -top-4 -right-4 flex h-[18px] w-[18px] items-center justify-center text-[18px] font-base tracking-wider text-[#F7941D]  leading-none">
                    {wishlistTotalCount < 10
                      ? `0${wishlistTotalCount}`
                      : wishlistTotalCount}
                  </span>
                )}
              </div>
            ...
            <Link
              href="/compare"
              className="relative flex items-center gap-2 tracking-tight font-medium text-[#ffffff] hover:opacity-80 transition"
            >
              <div className="relative flex items-center justify-center">
                <Image
                  src="/images/comparenav.svg"
                  alt="Compare"
                  width={24}
                  height={24}
                  className="w-6 h-6"
                ...
                <>
                  ...
                  <div className="absolute right-0 top-full mt-2 w-[350px] bg-white rounded-lg shadow-[0_10px_40px_rgba(0,0,0,0.12)] border border-slate-100 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                    ...
                    <div className="flex items-center justify-between px-4 py-3 border-b border-slate-50">
                      <h3 className="font-bold text-[#001E3C] text-[15px]">
                        Cart Items
                      </h3>
                      <button
                        onClick={() => setIsCartDropdownOpen(false)}
                    ...
                    <div className="max-h-[300px] overflow-y-auto [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1 [&::-webkit-scrollbar-thumb]:bg-slate-200">
                      {cartItems.length > 0 ? (
                        cartItems.map((item) => (
                          <div
                            key={item.id}
                            className="flex items-start gap-3 p-4 border-b border-slate-50 hover:bg-slate-50/50 transition-colors group/item"
                          >
                            ...
                            <div className="flex-1 min-w-0 pr-2">
                              <h4 className="text-[13px] font-bold text-[#001E3C] line-clamp-2 leading-tight mb-1">
                                {item.title}
                              </h4>
                    ...
                    {cartItems.length > 0 && (
                      <div className="p-4 bg-white">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <span className="text-[16px] font-bold text-[#001E3C]">
                ...
                <div className="group relative">
                  <button className="flex items-center gap-2 rounded-full border-2 border-gray-100 p-0.5 hover:border-[#2b85ff] transition-all duration-300">
                    <div className="h-10 w-10 overflow-hidden rounded-full border border-gray-50 bg-slate-50 flex items-center justify-center">
                      {(typeof user?.avatar_original === "string" &&
                        (user.avatar_original.startsWith("/") ||
                          user.avatar_original.startsWith("http"))) ||
