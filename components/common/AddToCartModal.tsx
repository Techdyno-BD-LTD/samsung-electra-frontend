...
export default function AddToCartModal({
  ...
  const title =
    ...
    productData?.name ||
    initialData?.name ||
    (isDemoMode ? "Product Title" : "");
  const brandLogo =
    fallbackBrandLogo || productData?.brand?.logo || "/images/samsung.png";
  ...
  const productSlug =
    initialSlug || initialData?.slug || (title ? toProductSlug(title) : "");
  const isWishlisted = useAppSelector((state) =>
    state.wishlist.items.some((item) => item.id === productSlug),
  );
  ...
  useEffect(() => {
    ...
    const targetSlug =
      initialSlug ||
      initialData?.slug ||
      (initialData?.name ? toProductSlug(initialData.name) : "");
  ...
  useEffect(() => {
    ...
    (Array.isArray(attrs) ? attrs : []).forEach(
      (attribute: {
        attribute_id?: number | string;
        name?: string;
        values?: string[];
  ...
  const emiDetailsLabel =
    productData?.emi_facility?.link_label || "See details";
  const colorLabel = "Color";
  const mainImage =
    fallbackImage || productData?.thumbnail_image || "/images/wm2.png";
  const warrantyBadgeImage = "/images/warrantybadge.png";
  const specialOfferLeft =
    productData?.special_offer_title || (isDemoMode ? "Special Offer" : "");
  const specialOfferOne =
    productData?.special_offers?.[0]?.text || (isDemoMode ? "Offer 1" : "");
  const specialOfferTwo =
    productData?.special_offers?.[1]?.text || (isDemoMode ? "Offer 2" : "");
  const showroomTitle =
    productData?.book_in_showroom_title ||
    (isDemoMode ? "Book in showroom Get 5% Off" : "");
  const shippingInfo =
    productData?.estimated_shipping_text ||
    (isDemoMode ? "Shipping information" : "");
  const warrantyInfo =
    productData?.warranty?.text ||
    (productData?.warranty?.warranty_type
      ? `Warranty: ${productData.warranty.warranty_type}`
      : isDemoMode
        ? "Warranty information"
        : "");
  const warrantyLinkLabel = productData?.warranty?.link_label || "View policy";
  const emiFacilityInfo =
    productData?.emi_facility?.text || (isDemoMode ? "EMI information" : "");
  const emiLinkLabel =
    productData?.emi_facility?.link_label || "See EMI Details";
  const exchangeInfo =
    productData?.exchange?.text || (isDemoMode ? "Exchange information" : "");
  const exchangeLinkLabel =
    productData?.exchange?.link_label || "Available Showroom";
  const madeInText =
    productData?.made_in_text || (isDemoMode ? "Product information" : "");

  const brandName = fallbackBrand || productData?.brand?.name || "Brand";
  const categoryName =
    fallbackCategory ||
    productData?.category_info?.category_name ||
  ...
  const attributesForUi = useMemo(() => {
    ...
    return (Array.isArray(attrs) ? attrs : []).filter(
      (attribute) => (attribute.values?.length ?? 0) > 0,
    );
  ...
  const fallbackColorDetails = useMemo(() => {
    ...
    return (Array.isArray(cols) ? cols : []).map(
      (color: string | { name?: string; value?: string; code?: string }) => ({
        name:
          typeof color === "string"
            ? color
  ...
  const colorOptions = useMemo(() => {
    return colorsForUi.map((color, index) => {
      ...
      const matchedVariant = variants.find((variant) => {
        ...
        return (
          variantName === colorNameComparable ||
          variantName.startsWith(`${colorNameComparable}-`)
        );
      });
      return {
        key: `${colorName}-${index}`,
        ...
        code: color.code,
        image:
          matchedVariant?.image ?? productData?.thumbnail_image ?? mainImage,
      };
    });
  }, [colorsForUi, variants, productData?.thumbnail_image, mainImage]);

  const activeColorName =
    selectedColorName || colorOptions[0]?.name || fallbackColor || "";
  const selectedVariant = variants.find((variant) => {
    ...
    if (
      comparableColor &&
      !(
        variantName === comparableColor ||
        variantName.startsWith(`${comparableColor}-`)
      )
    )
      return false;
  ...
  const selectedAvailability =
    (selectedVariant?.qty ?? productData?.current_stock ?? 0) > 0
      ? "In Stock"
      : "Out of Stock";
  ...
  const basePriceNum = selectedVariant
    ? Number(selectedVariant.price)
    : Number(productData?.calculable_price || 0);
  ...
  if (discountValue > 0) {
    if (discountType === "percent") {
      if (selectedVariant) {
        ...
        const stroked = productData?.stroked_price
          ? Number(String(productData.stroked_price).replace(/[^0-9.-]+/g, ""))
          : 0;
  ...
  const displayGallery = useMemo(() => {
    ...
    const thumb =
      productData?.thumbnail_image || mainImage || "/images/wm2.png";
    ...
    (Array.isArray(photos) ? photos : []).forEach(
      (p: { path?: string; photo?: string }) => {
    ...
    (Array.isArray(variantsData) ? variantsData : []).forEach(
      (v: { image?: string | null }) => {
  ...
  useEffect(() => {
    ...
    const selectedImage =
      ...
      colorOptions.find(
        (color) => toComparable(color.name) === toComparable(activeColorName),
      )?.image;
    if (selectedImage) {
  ...
  const buildCartPayload = () => {
    const targetSlug =
      initialSlug ||
      productData?.slug ||
      (title ? toProductSlug(title) : "product");
    ...
    return {
      ...
      originalPrice: originalPrice
        ? String(originalPrice).split("/")[0].trim()
        : "0",
      ...
      type: categoryName,
      weight:
        fallbackWeight || productData?.weight
          ? `${fallbackWeight || productData?.weight}kg`
          : "N/A",
      quantity: quantity,
        ...
        items: [
          {
            ...
            item_category:
              productData?.category_info?.parent_category_name ||
              cartPayload.type,
            item_category2: productData?.category_info?.parent_category_name
              ? cartPayload.type
              : undefined,
  ...
  return (
    <>
      <div
        className={`fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-1 transition-all duration-300 ${isHidingModal && !showSuccessModal ? "opacity-0 pointer-events-none" : "opacity-100"}`}
      >
        <div
          className={`relative w-full max-w-[1200px] max-h-[70vh] flex flex-col bg-white rounded-2xl shadow-2xl overflow-hidden transition-all duration-300 ${isHidingModal ? "scale-95 opacity-0 pointer-events-none" : "animate-in fade-in zoom-in-95 duration-200"}`}
            ...
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="w-full lg:w-[40%] space-y-4">
                <div className="relative rounded-2xl border border-slate-200 bg-white p-4">
                  <button
                    type="button"
                    className="absolute right-4 top-4 inline-flex h-8 w-8 items-center justify-center rounded-full border border-slate-300 bg-white text-slate-500 hover:bg-slate-50"
                  >
              ...
              <div className="w-full lg:w-[60%] space-y-2">
                <div>
                  ...
                  <h1 className="text-lg font-semibold text-slate-900  leading-tight mb-3 ">
                    {title}
                  </h1>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-1 text-xs text-slate-500">
                  ...
                  <span className="text-slate-300">|</span>
                  <span className="text-[#0A67C8] font-semibold">
                    {selectedAvailability}
                  </span>
                </div>

                <div className="flex items-center gap-6 ">
                  <p className="text-2xl font-semibold text-[#0C73DA]">
                    {price}
                  </p>
                  {saveAmountNum > 0 && (
                    <>
                      <div className="flex flex-col justify-center">
                        <p className="text-[12px] font-semibold text-[#15A85B] uppercase tracking-wide">
                          {discountLabel}
                        </p>
                ...
                <div className="flex items-center gap-3 py-2 text-sm text-slate-700">
                  ...
                  />
                  <span>
                    EMI Starts From <span className="font-bold">{emiText}</span>
                  </span>
                  <button
                    type="button"
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {colorOptions.map((color) => {
                        const isSelected =
                          toComparable(color.name) ===
                          toComparable(activeColorName);
                        return (
                          <button
                            key={color.key}
                            type="button"
                          ...
                          >
                            ...
                            />
                            <span
                              className={`text-xs font-bold ${isSelected ? "text-blue-700" : "text-slate-700"}`}
                            >
                              {color.name}
                            </span>
                          </button>
                ...
                <div className="flex flex-col gap-4 pt-2">
                  <div className="flex items-center gap-4">
                    ...
                    >
                      <FaHeart className={`h-[22px] w-[22px] ${isWishlisted ? "text-red-500" : "text-slate-400"}`} />
                    </button>
                    <button
                      type="button"
                      onClick={handleShare}
                      className="text-slate-600 hover:text-blue-500 transition-colors flex items-center"
                    >
                      <Image
                    ...
                    {showroomTitle && (
                      <Link
                        href="/stores"
                        className="text-sm font-bold text-[#0C73DA] hover:underline ml-auto"
                      >
                        {showroomTitle}
                      </Link>
                    )}
                ...
                <div className="space-y-4 pt-2">
                  <div className="grid grid-cols-2 gap-4 text-xs font-medium text-slate-600">
                    ...
                    <p className="flex items-center gap-2">
                      <Image
                        src="/images/freeinstallation.svg"
                        alt="Install"
                        width={18}
                        height={18}
                        className="h-4.5 w-4.5 opacity-80"
                      ...
                      <div className="rounded-xl border border-blue-100 bg-blue-50/30 p-3">
                        ...
                        <div className="flex flex-wrap gap-4">
                          {hasOffers ? (
                            offers.map(
                              (
                                offer: { image?: string | null; text?: string },
                                index: number,
                              ) => (
                                <div
                                  key={index}
