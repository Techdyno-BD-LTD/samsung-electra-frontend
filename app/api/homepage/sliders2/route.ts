...
export async function GET() {
	...
	try {
		const response = await fetch(`${baseUrl}/api/v2/sliders-two`, {
			method: "GET",
			cache: "no-store",
			headers: {
			...
			return NextResponse.json(
				{
					...
					message: "Failed to fetch sliders-two from backend.",
					backend: payload,
				},
				{ status: response.status }
			);
		...
		return NextResponse.json(
			{
				...
				success: false,
				status: 500,
				message: "Unable to reach backend sliders-two API.",
			},
			{ status: 500 }
		);
	}
