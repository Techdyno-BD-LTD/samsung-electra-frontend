...
export async function GET() {
  ...
  try {
    const response = await fetch(`${backendUrl}/api/v2/products/top-rated?limit=12`, {
      cache: 'no-store',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: "Failed to fetch top rated products" }, { status: response.status });
    }
