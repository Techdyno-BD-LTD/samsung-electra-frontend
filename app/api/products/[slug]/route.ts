...
export async function GET(
  ...
  try {
    const response = await fetch(`${backendUrl}/api/v2/products/${slug}`, {
      cache: 'no-store',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: `Failed to fetch product details for ${slug}` }, { status: response.status });
    }
