...
export async function GET() {
  ...
  try {
    const response = await fetch(`${backendUrl}/api/v2/higher-sale-products`, {
      cache: 'no-store',
      headers: {
    ...
    if (!response.ok) {
      return NextResponse.json({ error: 'Failed to fetch higher sale products' }, { status: response.status });
    }
