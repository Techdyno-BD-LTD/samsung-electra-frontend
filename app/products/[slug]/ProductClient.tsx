...
type ProductDetailsEntry = {
  ...
  colorDetails?: { name: string; code: string; image: string }[];
  features: string[];
  ...
  specialOfferOneImage?: string;
  specialOfferTwoImage?: string;
  specialOffers?: Array<{ text: string; image: string }>;
  shippingInfo: string;
  warrantyInfo: string;
  emiFacilityInfo: string;
  exchangeInfo: string;
  madeInText: string;
  variantPhotos?: { variant: string; path: string }[];
};
...
function parseHtmlFeatures(htmlContent?: string): string[] {
...
function mapApiProductToDetailsEntry(slug: string, apiProduct: any): ProductDetailsEntry {
  const brandName = apiProduct.brand?.name ?? "SAMSUNG";
  
  // Format specifications into an HTML table
  const specHtml = apiProduct.specifications && apiProduct.specifications.length > 0 
    ? `<table><tbody>${apiProduct.specifications.map((s: any) => `<tr><td>${escapeHtml(s.label)}</td><td>${escapeHtml(s.value)}</td></tr>`).join("")}</tbody></table>`
    : "";

  return {
    slug,
    category: apiProduct.category_info?.category_name ?? "Category",
    title: apiProduct.name ?? "Product",
    brand: brandName,
    brandLogo: apiProduct.brand?.logo ?? "/images/samsung.png",
    rating: apiProduct.reviews?.average ?? apiProduct.rating ?? 0,
    ratingCount: `(${apiProduct.reviews?.total ?? apiProduct.rating_count ?? 0})`,
    model: apiProduct.model_number ?? "N/A",
    sku: apiProduct.variants?.[0]?.sku ?? "N/A",
    availability: apiProduct.current_stock > 0 ? "In-stock" : "Stock Out",
    price: apiProduct.main_price ?? `৳${apiProduct.calculable_price ?? 0}`,
    originalPrice: apiProduct.stroked_price ?? apiProduct.main_price,
    discountLabel: apiProduct.discount ?? "0%",
    saveLabel: "Save", // Can be calculated if needed
    offersLabel: "View Offers",
    emiText: apiProduct.emi_start ? `EMI Starts from ${apiProduct.emi_start}` : "EMI Available",
    emiDetailsLabel: "See EMI Details",
    capacities: apiProduct.choice_options?.[0]?.options ?? [],
    colorLabel: "Color",
    colorSwatches: apiProduct.colors ?? [],
    colorDetails: (apiProduct.color_details ?? []).map((cd: any) => {
      // Find the first variant image matching this color name
      const colorName = cd.name?.toLowerCase() ?? "";
      const matchingPhoto = (apiProduct.photos ?? []).find((p: any) =>
        p.variant?.toLowerCase().startsWith(colorName)
      );
      const matchingVariant = (apiProduct.variants ?? []).find((v: any) =>
        v.variant?.toLowerCase().startsWith(colorName)
      );
      return {
        name: cd.name ?? "",
        code: cd.code ?? "",
        image: matchingPhoto?.path ?? matchingVariant?.image ?? apiProduct.thumbnail_image ?? "",
      };
    }),
    features: parseHtmlFeatures(apiProduct.other_features) || apiProduct.short_description_points ?? [],
    descriptionHtml: apiProduct.description ?? "",
    specificationsHtml: specHtml,
    featuresHtml: apiProduct.other_features ?? "",
    policyHtml: apiProduct.product_policy?.content ? `<h3>${escapeHtml(apiProduct.product_policy.title ?? "Product Policy")}</h3><div style="white-space: pre-line">${escapeHtml(apiProduct.product_policy.content)}</div>` : "",
    flashTitle: "Flash Sales Ends in",
    flashTime: "00 : 00 : 00 : 00",
    flashTimeLabels: "d : h : m : s",
    coupon: "",
    gallery: apiProduct.photos?.map((p: any) => p.path) || [apiProduct.thumbnail_image],
    mainImage: apiProduct.thumbnail_image ?? "",
    variantPhotos: apiProduct.photos ?? [],
    warrantyBadgeImage: "/images/warrantybadge.png",
    specialOfferLeft: apiProduct.special_offer_title || "Special Offer",
    specialOfferOne: apiProduct.special_offers?.[0]?.text || "Offer 1",
    specialOfferTwo: apiProduct.special_offers?.[1]?.text || "Offer 2",
    specialOfferOneImage: apiProduct.special_offers?.[0]?.image,
    specialOfferTwoImage: apiProduct.special_offers?.[1]?.image,
    specialOffers: apiProduct.special_offers || [],
    shippingInfo: apiProduct.estimated_shipping_text ?? "",
    warrantyInfo: apiProduct.warranty?.text ?? "",
    emiFacilityInfo: apiProduct.emi_facility?.text ?? "",
    exchangeInfo: apiProduct.exchange?.text ?? "",
    madeInText: apiProduct.made_in_text ?? "",
  };
}

export default function ProductDetailPage({ slug, initialData }: { slug: string; initialData?: any }) {

  let product: ProductDetailsEntry | null = null;
  
  if (initialData) {
    product = mapApiProductToDetailsEntry(slug, initialData);
  } else {
    ...
    fetch("http://localhost:3000/api/homepage/trust-signals")
      ...
      .then((json) => {
        if (json.success && Array.isArray(json.data)) {
          setTrustSignals(json.data);
        }
      })
      .catch((err) => console.error("Failed to fetch trust signals:", err));
  }, []);

  // Filter gallery images based on selected color
  const filteredGallery = useMemo(() => {
    if (!product) return [];
    const colorDetail = product.colorDetails?.[selectedColorIndex];
    if (!colorDetail || !product.variantPhotos?.length) return product.gallery;
    const colorName = colorDetail.name.toLowerCase();
    const colorPhotos = product.variantPhotos
      .filter((p) => p.variant?.toLowerCase().startsWith(colorName))
      .map((p) => p.path);
    return colorPhotos.length > 0 ? colorPhotos : product.gallery;
  }, [product, selectedColorIndex]);

  const currentMainImage = filteredGallery[selectedGalleryIndex] ?? product.mainImage;

  const handleColorSelect = (index: number) => {
    setSelectedColorIndex(index);
    setSelectedGalleryIndex(0); // Reset to first image of the new color
  };
              ...
              <div className="hidden grid-cols-5 gap-2 md:grid">
                {filteredGallery.map((item, index) => (
                  <button
                    ...
                    className={`rounded-2xl border p-2 transition-all ${index === selectedGalleryIndex ? "border-slate-700 bg-slate-100" : "border-slate-200 bg-white hover:border-slate-400"}`}
                  >
                    <Image
                      src={item}
            ...
            <div className="w-full lg:space-y-3 space-y-2  lg:w-[47%]">
                ...
                <div className="flex items-center gap-2 whitespace-nowrap">
                  ...
                  <div className="flex items-center gap-0.5 text-[#F59E0B]">
                    {Array.from({ length: 5 }, (_, i) => (
                      <FaStar key={i} className={`h-2.5 w-2.5 ${i < Math.floor(product.rating) ? "text-[#F59E0B]" : "text-slate-300"}`} />
                    ))}
              ...
              <div className="hidden flex-wrap items-center gap-3 text-sm text-slate-500 lg:flex">
                ...
                <div className="flex items-center gap-1 text-[#F59E0B]">
                  {Array.from({ length: 5 }, (_, i) => (
                    <FaStar key={i} className={`h-3 w-3 ${i < Math.floor(product.rating) ? "text-[#F59E0B]" : "text-slate-300"}`} />
                  ))}
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-3 text-[12px] lg:text-sm">
                  ...
                  {(product.colorDetails?.length ?? 0) > 0 ? (
                    product.colorDetails!.map((colorDetail, index) => (
                      <button
                        key={`color-${colorDetail.code}-${index}`}
                        type="button"
                        onClick={() => handleColorSelect(index)}
                        title={colorDetail.name}
                        className={`rounded border p-1 transition-all ${index === selectedColorIndex ? "border-slate-600 ring-1 ring-slate-400" : "border-slate-200 hover:border-slate-400"} flex items-center justify-center`}
                      >
                        {colorDetail.image && colorDetail.image.includes('/') ? (
                          <Image src={colorDetail.image} alt={colorDetail.name} width={36} height={36} className="lg:h-9 lg:w-9 w-6 h-6 object-contain" />
                        ) : (
                          <div style={{ backgroundColor: colorDetail.code }} className="lg:h-8 lg:w-8 w-5 h-5 rounded-sm border border-slate-300" />
                        )}
                      </button>
                    ))
                  ) : (
                    product.colorSwatches.map((colorItem, index) => {
                      const isColorCode = colorItem.startsWith('#') || !colorItem.includes('/');
                      return (
                        <button
                          key={`${colorItem}-${index}`}
                          type="button"
                          className={`rounded border p-1 ${index === 0 ? "border-slate-600" : "border-slate-200"} flex items-center justify-center`}
                        >
                          {isColorCode ? (
                            <div style={{ backgroundColor: colorItem }} className="lg:h-8 lg:w-8 w-5 h-5 rounded-sm border border-slate-300" />
                          ) : (
                            <Image src={colorItem} alt="Color option" width={36} height={36} className="lg:h-9 lg:w-9 w-6 h-6 object-contain" />
                          )}
                        </button>
                      );
                    })
              ...
              <div className="hidden lg:flex lg:flex-wrap lg:items-center lg:gap-x-3 lg:gap-y-2 lg:py-1 lg:text-md lg:text-slate-600">
                {trustSignals.length > 0 ? (
                  trustSignals.map((signal, index) => (
                    <p key={`trust-${index}`} className="mr-0 flex shrink-0 items-center gap-1.5 border-r border-slate-300 pr-3 last:border-r-0 last:pr-0 lg:mr-4 lg:gap-2 lg:border-r-2 lg:pr-4">
                      <Image 
                        src={signal.image} 
                        alt={signal.title} 
                        width={20} 
              ...
              {(product.specialOffers?.length ?? 0) > 0 ? (
                ...
                <div className="hidden lg:inline-flex lg:w-fit lg:max-w-full lg:flex-wrap lg:items-center lg:gap-2 lg:rounded-lg lg:border lg:border-[#2F7FE8] lg:px-3 lg:py-2 lg:text-sm lg:text-slate-900">
                  ...
                  <span className="flex items-center gap-2 whitespace-nowrap text-[15px] md:text-sm">
                    {product.specialOfferOneImage ? (
                      <Image src={product.specialOfferOneImage} alt="Offer 1" width={40} height={24} className="h-6 w-auto object-contain" />
                    ) : (
                  ...
                  <span className="flex items-center gap-2 whitespace-nowrap text-[15px] md:text-sm">
                    {product.specialOfferTwoImage ? (
                      <Image src={product.specialOfferTwoImage} alt="Offer 2" width={40} height={24} className="h-6 w-auto object-contain" />
                    ) : (
                      <Image src="/images/nagad.png" alt="Nagad" width={40} height={24} className="h-6 w-auto object-contain" />
                    )}
