...
type ShowroomCtaSectionProps = {
  ...
  description: string;
  buttonLabel: string;
  buttonHref: string; // Kept for prop compatibility but not used as link
};

export default function ShowroomCtaSection({ title, description, buttonLabel }: ShowroomCtaSectionProps) {
  ...
  return (
    <>
      ...
      <ExchangeModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} />
    </>
  );
}
